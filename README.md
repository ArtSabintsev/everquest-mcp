...
- **383 tools** for querying EverQuest data
...
| `get_class_recourse_profile` | Recourse spell analysis — secondary effects triggered on cast, recourse chains |
| `get_class_pushback_profile` | Pushback/knockup effects — displacement forces and strongest knockback abilities |
| `get_class_spell_recovery_profile` | Spell recovery times — global cooldown between casts, recovery brackets |
| `get_class_spell_overlap` | Compare two classes' spell overlap — shared/unique spells, level differences |
| `get_class_ae_range_profile` | AE range distribution — range brackets, AE by target type, largest radius abilities |
| `get_class_spell_density_map` | Spell density heat map — spells per 5-level bucket, peak levels, level gaps |
...
| `get_class_ae_capacity_profile` | AE capacity — PB AE, targeted AE, directional, beam, ring AE with ranges |
| `get_global_spell_statistics` | Global spell database stats — total spells, per-class counts, resist/target types |
| `get_class_defensive_cooldown_profile` | Defensive cooldowns — 30s+ recast beneficial abilities, disciplines, emergencies |
| `get_class_offensive_cooldown_profile` | Offensive cooldowns — 30s+ recast detrimental burns and high-cooldown attacks |
| `get_class_spell_power_curve` | Spell power curve — max damage, healing, mana cost per 10-level bracket |
| `get_class_spells_by_expansion` | Spells by expansion era — Classic through modern expansion distribution |
| `get_class_spell_timer_conflict_analysis` | Timer conflict analysis — spells sharing timer IDs (can't use simultaneously) |
| `get_class_spell_global_ranking` | Global class ranking — rank among 16 classes on spells, SPAs, damage, healing |
| `get_class_resource_cost_comparison` | Resource cost comparison — avg/max mana and endurance by level bracket |
...
| `get_class_spell_by_level_range` | Query spells in a level range with per-level breakdown |
| `get_global_spa_distribution` | Top 50 most common SPAs across all 70K+ spells |
| `get_spa_class_matrix` | Which classes use a specific SPA — counts and level ranges |
...
| `get_class_dot_efficiency` | DoT efficiency — damage over time ranked by total damage and damage per mana |
| `get_class_burst_damage_window` | Burst damage — max damage in 6s, 12s, and 18s burst windows |
| `get_class_sustained_dps_profile` | Sustained DPS — nukes ranked by damage per cycle time (cast + recast) |
| `get_class_spell_by_effect_value` | Filter spells by SPA effect ID and minimum base value threshold |
| `get_class_new_spells_by_level_bracket` | New vs upgrade spells by level bracket — new spell lines vs upgrades |
| `get_class_obsolete_spell_analysis` | Identify spells superseded by strictly better versions in the same line |
